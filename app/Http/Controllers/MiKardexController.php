 ...
 * El rol `alumno` ya traía `ver-kardex`, pero el único sitio donde se pinta un
 * kárdex es el expediente de control escolar, y ése cuelga de `ver-alumnos`
 ...
 * Nunca de la URL. No hay forma de pedir el kárdex de otro porque no hay dónde
 * escribir su id. Quien estudia dos carreras elige entre las SUYAS, y la
 * elección se comprueba contra esa misma lista.
 */
class MiKardexController extends Controller
{
    ...
    public function __invoke(Request $peticion): Response
    {
        ...
        // Sin matrícula no hay kárdex: es alguien con el permiso pero sin ser
        // alumno todavía. Se dice, en vez de pintar una tabla vacía que parece
        // que se perdió su historial.
        if ($matriculas->isEmpty()) {
            return Inertia::render('MiKardex', [
                'matriculas' => [],
                'matricula' => null,
                'renglones' => [],
                'resumen' => null,
         ...
         * del resumen— y la escala con la que `KardexDelAlumno::promedio`
        ...
        return Inertia::render('MiKardex', [
            ...
            'renglones' => $this->kardex->renglones($elegida),
            'resumen' => $this->kardex->resumen($elegida),
        ]);
    }
