...
| 6.3 Compras / cuentas por pagar | **EN CURSO** (era E) — decidido con el cliente: compras completo en 3 rebanadas, NO contabilidad. **Rebanada 1 (Proveedores) ✅**; CxP y Órdenes de compra pendientes. Diseño en `docs/plan-compras-cxp.md` | rebanada 1: `proveedores` + `egresos.proveedor_id`; el ejercido sigue siendo sólo `egresos` | proveedores→CxP→OC, ligado a presupuesto/centro de costo/egreso; sin duplicar el gasto | el ejercido no se duplica; RFC único; `prueba-proveedores.php` (14 verif, 5 mut) |
