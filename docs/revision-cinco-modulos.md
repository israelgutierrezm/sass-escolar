...
| 6.3 Compras / cuentas por pagar | **EN CURSO** (era E) — decidido con el cliente: compras completo en 3 rebanadas, NO contabilidad. **Rebanada 1 (Proveedores) ✅ · Rebanada 2 (Cuentas por pagar) ✅**; Órdenes de compra pendiente. Diseño en `docs/plan-compras-cxp.md` | r1: `proveedores`+`egresos.proveedor_id`; r2: `cuentas_por_pagar`, `RegistradorPagoProveedor` (pagar→egreso, saldo/estado derivados), antigüedad de saldos; el ejercido sigue siendo sólo `egresos` | proveedores→CxP→OC, ligado a presupuesto/centro de costo/egreso; sin duplicar el gasto | pagar asienta UN egreso (no duplica el ejercido); no se paga de más (bajo bloqueo); `prueba-proveedores.php` (14/5) + `prueba-cuentas-por-pagar.php` (25 verif, 8 mut) |
