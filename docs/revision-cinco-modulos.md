...
| 6.3 Compras / cuentas por pagar | **✅ HECHO** (era E) — compras completo en 3 rebanadas, NO contabilidad. **Proveedores ✅ · Cuentas por pagar ✅ · Órdenes de compra ✅**. Diseño en `docs/plan-compras-cxp.md` | r1: `proveedores`+`egresos.proveedor_id`; r2: `cuentas_por_pagar`, `RegistradorPagoProveedor` (pagar→egreso, saldo/estado derivados), antigüedad de saldos; r3: `ordenes_compra`+conceptos, `GestorDeOrdenesCompra` (autorizar→recibir→genera CxP); el ejercido sigue siendo sólo `egresos` | proveedores→OC→CxP, ligado a presupuesto/centro de costo/egreso; sin duplicar el gasto | pagar asienta UN egreso; recibir genera la CxP (no crea egreso); no se paga/recibe de más (bajo bloqueo); `prueba-proveedores.php` (14/5) + `prueba-cuentas-por-pagar.php` (25/8) + `prueba-ordenes-compra.php` (21 verif, 7 mut) |
...
| 7.2 Salida segura · rebanada 1 (registro y reglas) | **✅ HECHO** (era E) | `autorizados_recoger`; `PuedeRecoger` (bloqueo > tutor > autorizado vigente); familia agrega terceros, escuela registra bloqueos de custodia; permiso `gestionar-salida-segura`. Diseño en `docs/plan-salida-segura.md` | — | implementado: quién puede recoger, con custodia que vence a todo | `prueba-salida-segura.php` (19 verif, 6 mutaciones) |
| 7.2 Salida segura · rebanada 2 (la puerta) | **✅ HECHO** (era E) | `token` (QR) por autorizado, generado por el servidor sólo en autorizaciones; `SalidaAlumno` (registro con quién/cuándo/cómo); `RegistradorDeSalida` (valida con `PuedeRecoger`, anota y avisa); pantalla del guardia `/plataforma/puerta`; permiso propio `registrar-salida-alumno` | — | implementado: QR validado en servidor sobre el estado ACTUAL (bloqueo posterior lo rechaza, vencido lo rechaza, token de un alumno no sirve para otro), dos caminos (QR o lista), aviso a la familia | `prueba-salida-segura-puerta.php` (26 verif, 6 mutaciones) |
