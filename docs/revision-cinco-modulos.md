...
| 3.4 Prerrequisitos ENTRE actividades | **E** | `Seriacion` existe pero es entre `plan_materias` (materias), no entre actividades del curso | una actividad no se puede bloquear hasta completar otra | tabla de aristas actividad→actividad con prevención de ciclos y explicación del bloqueo | ciclo rechazado, ruta imposible, motivo visible |
...
| 4.1 Supervisor externo (acceso) | **E** | sólo `OrganizacionContacto` (un contacto de la org, sin cuenta ni acceso); no hay invitación/activación/revocación ni alcance a expedientes | el supervisor no puede validar horas/reportes desde fuera | acceso acotado por invitación con token, vigencia y revocación, ligado a (org, plaza, expediente), **sin cartera ni datos ajenos** | supervisor sólo ve lo asignado; token caduca/revoca |
...
| 5.1 Simular reglas antes de activar | **E** | el motor corre en real; no hay modo «qué pasaría si» sin escribir alertas | no se puede calibrar un umbral sin generar señales | `permanencia:evaluar --simular` que devuelve a cuántos alcanzaría SIN persistir ni notificar | simulación no escribe una sola fila |
