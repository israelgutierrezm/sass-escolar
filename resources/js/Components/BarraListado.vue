...
<template>
    <section class="tarjeta space-y-3 p-4">
        <!-- Fila 1: buscador + botón «Filtros» + «Nuevo». Homóloga en todos los
             listados: una sola línea para el buscador y los botones. -->
        ...
        <!-- Fila 2: los filtros en su propia línea, DEBAJO del buscador, cuando
             se abren. Así no quedan apretados a la derecha del buscador. -->
        <div v-if="abierto" class="flex flex-wrap items-center gap-3 border-t pt-3" :style="{ borderColor: 'var(--color-borde)' }">
            <select
                v-for="f in filtros"
                :key="f.clave"
