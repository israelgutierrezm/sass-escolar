...
const mensajeVacio = computed(() =>
    props.filtros.busqueda || props.filtros.programa_academico_id || props.filtros.vigente
        ? 'Ningún plan coincide con la búsqueda.'
        : 'Aún no hay planes de estudio. Primero registra un programa académico.',
);
...
<template>
    ...
    <AppLayout titulo="Catálogo académico">
        ...
        <BarraListado
            ...
            descripcion="Mallas curriculares por programa académico"
            :icono="ICONO_PLAN"
        >
            <template #conteo>
        ...
        <template v-if="vista === 'cuadricula'">
            <section v-if="!vacio" class="cuadricula-listado">
                <TarjetaListado
                    ...
                    :clave="plan.clave"
                    :metas="[
                        { etiqueta: 'Programa académico', valor: plan.programaAcademico },
                ...
                <table v-if="!vacio" class="w-full text-sm">
                    ...
                    <tbody>
                        <tr v-for="plan in planes.data" :key="plan.id" class="fila-nueva border-t transition-colors" :style="{ borderColor: 'var(--color-borde)' }">
                            ...
                            <td class="px-4 py-4" :style="{ color: 'var(--color-suave)' }">{{ plan.programaAcademico ?? '—' }}</td>
                            <td class="px-4 py-4">
